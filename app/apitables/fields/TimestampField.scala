 ...
 */
class TimestampField(builder: FieldBuilder) extends Field(builder) {
  ...
  override private[apitables] def getValue(theName: String, resultSet: ResultSet): FieldValue = {
    ...
    if (resultSet.wasNull())
      new TimestampValue(None)
    else
      new TimestampValue(Some(d))
  }

  override private[apitables] def getValue(theName: String, params: Map[String, Any]): FieldValue =
    TimestampValue(getParamValue(theName, params))
}

class TimestampFB(name: String) extends FieldBuilder(name) {

  override def build: Field = new TimestampField(this)
}

object TimestampFB {
  def apply(name: String): TimestampFB = new TimestampFB(name)
}
